...
static int print_events(int fd)
{
	...
	while (1) {
		...
		for (i = 0; i < rd / sizeof(struct input_event); i++) {
			...
			// Alan's custom output format goes here
			if(1){
				// if (type == EV_MSC && (code == MSC_RAW || code == MSC_SCAN))
				 if(((type == EV_KEY) || (type == EV_ABS))
					&& !((type == EV_KEY) && (ev[i].value == 2)))
				
					if(type == EV_KEY)
						printf("%s\tEVENT\t%s\t%s\t%s\n", ident, codename(type, code), typename(type), (ev[i].value == 0) ? "OFF" : "ONN");
	    				else
						printf("%s\tEVENT\t%s\t%s\t%d\n", ident, codename(type, code), typename(type), ev[i].value);
				
					// printf("EVENT\t%s\t%d\t%s\t%d\n", typename(type), code, codename(type, code), ev[i].value);
...
static const struct option long_options[] = {
	...
	{ "ident", required_argument, NULL, 1984 },
	{ 0, },
};
